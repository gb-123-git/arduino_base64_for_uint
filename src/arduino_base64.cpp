...
void data_base64::encode(const uint8_t* input, size_t inputLength, char* output){
    uint8_t position = 0;
    uint8_t bit8x3[3] = {};
    uint8_t bit6x4[4] = {};
...
size_t data_base64::encodeLength(size_t inputLength){
    return (inputLength + 2 - ((inputLength + 2) % 3)) / 3 * 4 + 1;
}

void data_base64::decode(const char* input, uint8_t* output){
...
size_t data_base64::decodeLength(const char* input){
    auto inputLength = strlen(input);
    uint8_t equal = 0;
